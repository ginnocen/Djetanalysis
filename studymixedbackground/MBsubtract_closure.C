...
int MBsubtract_closure(std::string Djetfile, std::string mixjetfile, std::string mixDfile, std::string mixbothfile, std::string genfile, std::string output, Float_t jetptmin, Float_t jetetamin, Float_t jetetamax)
{
    ...
    for(int i=0;i<nPtBins;i++)
    {
        ...
        TH2F* hemptydr = new TH2F("hemptydr",";;#frac{1}{N_{evt}} #frac{dN_{evt}}{d#DeltaR}",1,0.,0.5,1,pow(10.,-5),pow(10.,1));
        TH2F* hemptydphi = new TH2F("hemptydphi",";;#frac{1}{N_{evt}} #frac{dN_{evt}}{d#Delta#phi}",1,0,TMath::Pi(),1,mixboth_dphi[i]->GetMinimum(),Djet_dphi[i]->GetMaximum());
        ...
        if(plotdr)
        {
            ...
            ldr[i] = new TLegend(0.45,0.6,0.95,0.9);
            ldr[i]->AddEntry(Djet_dr[i],"Signal D, Raw Jet","lp");
            ldr[i]->AddEntry(mixjet_dr[i],"Signal D, Mixed Jet","lp");
            ldr[i]->AddEntry(mixD_dr[i],"Mixed D, Raw Jet","lp");
            ldr[i]->AddEntry(mixboth_dr[i],"Mixed D, Mixed Jet","lp");
            ldr[i]->AddEntry(gen_dr[i],"Signal D, Signal Jet (subid==0)","lp");
            ldr[i]->AddEntry(MBsub_dr[i],"MB-subtracted","lp");
        ...
        if(plotdphi)
        {
            ...
            ldphi[i] = new TLegend(0.45,0.6,0.95,0.9);
            ldphi[i]->AddEntry(Djet_dr[i],"Signal D, Raw Jet","lp");
            ldphi[i]->AddEntry(mixjet_dr[i],"Signal D, Mixed Jet","lp");
            ldphi[i]->AddEntry(mixD_dr[i],"Mixed D, Raw Jet","lp");
            ldphi[i]->AddEntry(mixboth_dr[i],"Mixed D, Mixed Jet","lp");
            ldphi[i]->AddEntry(gen_dr[i],"Signal D, Signal Jet (subid==0)","lp");
            ldphi[i]->AddEntry(MBsub_dr[i],"MB-subtracted","lp");
