...
if [[ $# -ne 9 ]]; then
    echo "usage: ./skim-condor.sh [input dir] [output dir] [max jobs] [log dir] [mix filelist] [isPP] [isMC] [jetptmin] [check input empty]"
    exit 1
fi
...
do
    ...
    then
        ...
        then
            [[ $counter_mix -eq $nMIX_FILE ]] && { counter_mix=0 ; }
            echo -e "\033[38;5;242mSubmitting a job for output\033[0m ${DESTINATION}/${OUTFILE}_$i"
            infn=`echo $i | awk -F "." '{print $1}'`
            INFILE="${DATASET}/$i"
            echo ${MIX_FILE[$counter_mix]}
            echo ${MIX_FILE[$counter_mix]} > $LOGDIR/listmixing_${infn}.list
...
Arguments    = $INFILE $DESTINATION ${OUTFILE}_${infn}.root listmixing_${infn}.list $isPP $isMC $JETPTMIN $PROXYFILE
GetEnv       = True
...
transfer_input_files = D_jet_skim.exe,$LOGDIR/listmixing_${infn}.list,/tmp/$PROXYFILE
