...
if [[ $# -ne 8 ]]; then
    echo "usage: ./skim-djet.sh [input file] [output dir] [output filename] [mix file] [isPP] [isMC] [jetptmin] [proxy]"
    exit 1
fi
...
echo ./D_jet_skim.exe $INFILE $OUTFILE $isPP $isMC $JETPTMIN 0 -1 0 $MIX_FILE
./D_jet_skim.exe $INFILE $OUTFILE $isPP $isMC $JETPTMIN 0 -1 0 $MIX_FILE
