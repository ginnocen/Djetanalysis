...
void weightDpt_savehist(TString inputname, TString outputname, TString collisionsyst, int maxevt=-1)
{
  ...
  for(int i=0;i<rnentries;i++)
    {
      if(i%10000==0) std::cout<<std::setiosflags(std::ios::left)<<"  [ \033[1;36m"<<std::setw(10)<<i<<"\033[0m"<<" / "<<std::setw(10)<<rnentries<<" ] "<<"\033[1;36m"<<std::setw(4)<<Form("%.0f%s",100.*i/rnentries,"%")<<"\033[0m"<<"   >>   weightDpt_savehist("<<std::setw(20)<<Form("%s)",collisionsyst.Data())<<"\r"<<std::flush;
      //
  ...
  std::cout<<std::setiosflags(std::ios::left)<<"  Processed "<<"\033[1;31m"<<rnentries<<"\033[0m out of\033[1;31m "<<nentries<<"\033[0m event(s)."<<"   >>   weightDpt_savehist("<<std::setw(30)<<Form("%s)",collisionsyst.Data())<<std::endl;
  std::cout<<std::endl;
...
int main(int argc, char* argv[])
{
  if(argc==5)
    {
      weightDpt_savehist(argv[1], argv[2], argv[3], atoi(argv[4]));
      return 0;
    }
  else
    {
      std::cout<<"  Error: invalid arguments number - weightDpt_savehist()"<<std::endl;
      return 1;
    }
}
