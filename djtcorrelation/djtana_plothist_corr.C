                     ...
                     Int_t plotref = 1)
{
  ...
  TFile* infhist = new TFile(Form("%s",inputhistname.Data()));
...
//  std::vector<TH1F*>               hJetHists     =  {(TH1F*)hJetPhi,         (TH1F*)hJetEta};
  ...
  std::vector<Float_t>             jetmaxes      =  {2*(Float_t)TMath::Pi(), 2.};
...
//  std::vector<TH1F**>              hDHists       =  {(TH1F**)hDPhi,          (TH1F**)hDEta,                   (TH1F**)hDdelPhi,         (TH1F**)hDdelEta};   
  std::vector<Float_t>             Dmins         =  {0.,                     -2.,                             -2*(Float_t)TMath::Pi(),  -4.};
  std::vector<Float_t>             Dmaxes        =  {2*(Float_t)TMath::Pi(), 2.,                              2*(Float_t)TMath::Pi(),   4.};   
...
/*
  for(int i=0;i<2;i++)
    {
      ...
      c->SaveAs(Form("%s.pdf",hJetHists[i]->GetName()));
      delete c;
      delete hempty;
    }
  for(int i=0;i<4;i++)
      ...
      for(int j=0;j<nPtBins;j++)
        {
          ...
          xjjroot::drawCMS(collisionsyst);
          c->SaveAs(Form("%s_pt_%d.pdf",(hDHists.at(i))[j]->GetName(),j));
          delete c;
          delete hempty;
        }
    }
  for(int i=0;i<nPtBins;i++)
    {
      TCanvas* c = new TCanvas("c","",600,600);
      hCorr[i]->Draw("surf1 fb");
      c->SaveAs(Form("Correlation_pt_%d.pdf",i));
      delete c;
    } 
*/     
  for(int k=0;k<2;k++)
