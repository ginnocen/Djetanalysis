...
// std::vector<std::vector<Float_t>>* corrFactor = 0;
// std::vector<std::vector<Float_t>>* corrFactorSg = 0;
...
namespace djtcorr
{
  ...
  void setParameters(int ispp)
  {
    ...
    paramfResoDeltaPhi = ispp?&paramfResoDeltaPhi_pp:&paramfResoDeltaPhi_PbPb;
    paramfResoDeltaEta = ispp?&paramfResoDeltaEta_pp:&paramfResoDeltaEta_PbPb;

    // corrFactor = ispp?&corrFactor_pp:&corrFactor_PbPb;
    ...
    // corrFactorSg = ispp?&corrFactorSg_pp:&corrFactorSg_PbPb;
  ...
  int ptCorr(int jescale, float &jetpt, float jetnpfpart, int ibincent)
  {
    ...
    else { std::cout<<"invalid jescale"<<std::endl; return 1; }
  }
  ...
  int ptSmear(float &jetpt, int ibincent, int jereso=1)
  {
    ...
    std::cout<<"invalid jereso"<<std::endl; 
    return 1;
  }
  ...
  int angleSmear(int gensmearphi, float &jeteta, float &jetphi, float jetpt, int ibincent)
  {
    ...
    else { std::cout<<"invalid gensmearphi"<<std::endl; return 1; } 
  }
