...
TFile*fcross=new TFile("fcross.root","recreate");  
  ...
  TF1 *myTotal[samples_][ntriggers_];
	for (int index=0;index<samples_;index++){
		...
		cout<<expmyweightL1HLTfinal[index][indextriggers].Data()<<endl;
		  ...
		  myTotal[index][indextriggers]= new TF1("myfTotal",expmyL1HLTtest[index][indextriggers].Data(),0,1000);
          ...
          myTotal[index][indextriggers]->Write(Form("myTotal_%s",nametriggerselectiontagtriggers[index][indextriggers].Data()));		    		
        }
    }
fcross->Close();
}