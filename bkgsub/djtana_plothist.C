                     ...
                     Float_t jetptmin, Float_t jetetamin, Float_t jetetamax)
{
  ...
  for(int i=0;i<nPtBins;i++)
    {
      ahSignalRPH[0][i]->Scale(1./hnjet->GetBinContent(1), "width");
      ahSignalRPH[1][i]->Scale(1./hnjet->GetBinContent(1), "width");
      ahSignalRP[i]->Scale(1./hnjet->GetBinContent(1), "width");
      ahSignalRPHsub[i]->Scale(1./hnjet->GetBinContent(1), "width");
  ...
  for(int i=0;i<nPtBins;i++)
    {
      ...
      pXsec->cd();
      pXsec->SetLogy();
      TH2F* hempty = new TH2F("hempty", ";#DeltaR;#rho(#DeltaR)", 5, drBins[0], drBins[nDrBins], 10, yaxismin, yaxismax);
      ...
      xjjroot::setthgrstyle(ahSignalRPH[1][i], kRed-9, 25, 1.2, kRed-9, 2, 1, -1, -1, -1);
      ahSignalRPH[1][i]->Draw("pe same");
      xjjroot::setthgrstyle(ahSignalRP[i], kBlack, 20, 1.2, kBlack, 1, 1, -1, -1, -1);
      ...
      leg->AddEntry(ahSignalRPH[1][i], "relected cone", "p");
      ...
      pPull->Draw();
      pPull->cd();
      TH2F* hemptyPull = new TH2F("hemptyPull", ";#DeltaR;bkg sub / subid=0", 5, drBins[0], drBins[nDrBins], 10, 0, 2);
      ...
      c->SaveAs(Form("plotclosure/cbkgclosure_xsec_dr_%s_pt_%s_%s.pdf",outputname.Data(),xjjc::number_to_string(ptBins[i]).c_str(),xjjc::number_to_string(ptBins[i+1]).c_str()));
...
int main(int argc, char* argv[])
{
  if(argc==6)
    {
      djtana_plothist(argv[1], argv[2], atof(argv[3]), atof(argv[4]), atof(argv[5]));
      return 0;
    }
  else
    {
