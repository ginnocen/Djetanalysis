    ...
    do
        ...
        do
            ...
            then
                ...
                ./djtana_savehist.exe "${INPUTDANAME[i]}" "rootfiles/hist_${tPOSTFIX}" "${COLSYST[i]}" ${ISMC[i]} $k ${JETPTMIN[j]} ${JETPTMAX[j]} ${JETETAMIN[j]} ${JETETAMAX[j]} $ifScale $ifSmearPt $ifSmearPhi $SIGNALMC $MAXEVT $doDgenweight &
                echo
            fi
        done
    done
    ...
    do
        ...
        do
            ...
            do
                ...
                ./djtana_usehist.exe "rootfiles/hist_${tPOSTFIX}" "rootfiles/masstpl_${tPOSTFIX}" "$tPOSTFIX" "${COLSYST[i]}" $k ${JETPTMIN[j]} ${JETPTMAX[j]} ${JETETAMIN[j]} ${JETETAMAX[j]} $SIGNALMC $CORRRESO $CORRFACTOR
                echo
            done
        done
    done
