                      ...
                      TString collisionsyst)
{
  ...
  xjjroot::drawCMS(collisionsyst); 
  legScalePtCorr->Draw();
  cScalePtCorr->SaveAs(Form("plotresos/cScalePtCorr_%s.pdf",outputname.Data()));
  ...
  for(int k=0;k<nCentBins;k++)
    {
      ...
      if(ispp) fResoPt->FixParameter(2, 0);
  ...
  for(int k=0;k<nCentBins;k++)
    {
      ...
      TH2F* hemptyResoPtCorr = new TH2F("hemptyResoPtCorr", ";Gen jet p_{T} (GeV/c);#sigma(p_{T}^{reco} / p_{T}^{gen})", 10, jtptBins[0], jtptBins[nJtptBins], 10, 0, 0.4);
