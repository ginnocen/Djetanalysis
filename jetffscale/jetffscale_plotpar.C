                        ...
                        TString collisionsyst)
{
  ...
  TH2F* hemptyScaleP0NpfPt = new TH2F("hemptyScaleP0NpfPt", Form(";jet p_{T}^{reco} (GeV/c);a_{0}"), 10, jtptBins[0], jtptBins[nJtptBins], 10, ispp?150:20, ispp?400:250);
  TH2F* hemptyScaleP1NpfPt = new TH2F("hemptyScaleP1NpfPt", Form(";jet p_{T}^{reco} (GeV/c);a_{1}"), 10, jtptBins[0], jtptBins[nJtptBins], 10, ispp?-0.007:-0.04, ispp?-0.002:0);
  ...
  for(int k=0;k<nCentBins;k++)
    {
      ...
      TF1* fP0 = new TF1("fP0", "[0]-exp([1]+[2]*x)", jtptBins[0], jtptBins[nJtptBins]);
      fP0->SetLineWidth(3);
      fP0->SetLineColor(kOrange-9);
      fP0->SetLineStyle(1);
      TF1* fP1 = new TF1("fP1", "[0]-exp([1]+[2]*x+[3]*x*x)", jtptBins[0], jtptBins[nJtptBins]);
      fP1->SetLineWidth(3);
      ...
      hScaleP0NpfPt[k]->Fit("fP0", "L q", "", jtptBins[0], jtptBins[nJtptBins]);
      hScaleP0NpfPt[k]->Fit("fP0", "m q", "", jtptBins[0], jtptBins[nJtptBins]);
      hScaleP0NpfPt[k]->Fit("fP0", "m q", "", jtptBins[0], jtptBins[nJtptBins]);
      ...
      hScaleP1NpfPt[k]->Fit("fP1", "L q", "", jtptBins[0], jtptBins[nJtptBins]);
      hScaleP1NpfPt[k]->Fit("fP1", "m q", "", jtptBins[0], jtptBins[nJtptBins]);
      hScaleP1NpfPt[k]->Fit("fP1", "m q", "", jtptBins[0], jtptBins[nJtptBins]);
      xjjroot::drawCMS(collisionsyst); 
      texxpos = 0.22; texypos = 0.85; texdypos = 0.06; texypos += texdypos;
      xjjroot::drawtex(texxpos, texypos=(texypos-texdypos), "|#eta^{reco jet}| < 1.6");
      if(!ispp) xjjroot::drawtex(texxpos, texypos=(texypos-texdypos), texcent);
      xjjroot::drawtex(0.22, 0.2, Form("%.4f-exp(%s%.4f%s%.4f*x%s%.4f*x*x)",
                                       fP1->GetParameter(0),
  ...
  for(int k=0;k<nCentBins;k++)
    ...
    // std::cout<<"{"<<paramfP0[k].at(0)<<", "<<paramfP0[k].at(1)<<", "<<paramfP0[k].at(2)<<", "<<paramfP1[k].at(3)<<"}"<<(k==(nCentBins-1)?"};":",")<<std::endl;
