...
void eff_savetpl(TString inputname, TString collisionsyst, TString name,
                 TString outputname, Int_t irecogen,
                 ...
                 Int_t maxevt=-1)
{
  ...
  djtweight::init();
  Bool_t ifweightDpt = name.Contains("Dpt");
  ...
  for(int i=0;i<rnentries;i++)
    {
      if(i%10000==0) std::cout<<std::setiosflags(std::ios::left)<<"  [ \033[1;36m"<<std::setw(10)<<i<<"\033[0m"<<" / "<<std::setw(10)<<rnentries<<" ] "<<"\033[1;36m"<<std::setw(4)<<Form("%.0f%s",100.*i/rnentries,"%")<<"\033[0m"<<"   >>   eff_savetpl("<<std::setw(5)<<Form("%s,",collisionsyst.Data())<<" "<<std::setw(20)<<Form("%sD_%sjet)", djt.aDopt[irecogen].Data(), djt.ajetopt[irecogen].Data())<<"\r"<<std::flush;
      //
      ...
      Float_t pthatweight = name.Contains("pthatweight")?djt.pthatweight:1; 
      Float_t evtweight = pthatweight*cweight;
              ...
              for(int jd=0;jd<*(djt.anD[irecogen]);jd++)
                {
                  ...
                  Float_t weightDgen = ifweightDpt&&Dgenpt>0?djtweight::getDptweight(Dgenpt, ispp):1.;
              ...
              for(int jd=0;jd<djt.Gsize;jd++)
                {
                  ...
                  Float_t weightG = ifweightDpt&&Gpt>0?djtweight::getDptweight(Gpt, ispp):1.;
  ...
  std::cout<<std::setiosflags(std::ios::left)<<"  Processed "<<"\033[1;31m"<<rnentries<<"\033[0m out of\033[1;31m "<<nentries<<"\033[0m event(s)."<<"   >>   eff_savetpl("<<std::setw(5)<<Form("%s,",collisionsyst.Data())<<" "<<std::setw(30)<<Form("%sD_%sjet)", djt.aDopt[irecogen].Data(), djt.ajetopt[irecogen].Data())<<std::endl;
  std::cout<<std::endl;
...
int main(int argc, char* argv[])
{
  if(argc==14)
    {
      eff_savetpl(argv[1], argv[2], argv[3], 
                  argv[4], atoi(argv[5]),
    ...
    {
      std::cout<<"  Error: invalid arguments number - eff_savetpl()"<<std::endl;
      return 1;
    }
}
...
int arguerr(TString collisionsyst, Int_t irecogen)
{
  ...
  if(irecogen==1 || irecogen==3)
    {
      std::cout<<"\"irecogen\" indicates genD, skip eff_savetpl()."<<std::endl;
      return 2;
    }
  return 0;
}
