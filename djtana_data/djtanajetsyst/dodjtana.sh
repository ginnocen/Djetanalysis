...
do
    ...
    do
        ...
        do
            ...
            for s in ${sJESJER[@]}
            do
                tPOSTFIX=Djet_$(produce_postfix $i $j $k $s)
                if [[ $DO_SAVETPL -eq 1 ]]
                then
                    echo -e "-- Processing ${FUNCOLOR}djtana_savetpl.C${NC} :: ${ARGCOLOR}${COLSYST[i]}${NC} - ${ARGCOLOR}${tMC[${ISMC[i]}]}${NC} - ${ARGCOLOR}${RECOGEN[k]}${NC}"
                    ./djtana_savetpl.exe "${INPUTMCNAME[i]}" "rootfiles/masstpl_${tPOSTFIX}" "${COLSYST[i]}" ${ISMC[i]} $k ${JETPTMIN[j]} ${JETPTMAX[j]} ${JETETAMIN[j]} ${JETETAMAX[j]} ${ifScale[s]} $ifSmearPt $ifSmearPhi $MAXEVT &
                    echo
                fi
                if [[ $DO_SAVEHIST -eq 1 ]]
                then
                    echo -e "-- Processing ${FUNCOLOR}djtana_savehist.C${NC} :: ${ARGCOLOR}${COLSYST[i]}${NC} - ${ARGCOLOR}${tMC[${ISMC[i]}]}${NC} - ${ARGCOLOR}${RECOGEN[k]}${NC}"
                    ./djtana_savehist.exe "${INPUTDANAME[i]}" "rootfiles/hist_${tPOSTFIX}" "${COLSYST[i]}" ${ISMC[i]} $k ${JETPTMIN[j]} ${JETPTMAX[j]} ${JETETAMIN[j]} ${JETETAMAX[j]} ${ifScale[s]} $ifSmearPt $ifSmearPhi $SIGNALMC $MAXEVT &
                    echo
                fi
            done
        done
